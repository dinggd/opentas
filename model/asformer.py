...
class AttLayer(nn.Module):
    ...
    def construct_window_mask(self):
        ...
        for i in range(self.bl):
            # NOTE: Edited to address https://github.com/ChinaYi/ASFormer/issues/2 as mentioned in https://github.com/ChinaYi/ASFormer/blob/main/README.md
    ...
    def _block_wise_self_att(self, q, k, v, mask):
        ...
        if L % self.bl != 0:
            q = torch.cat(
                [q, torch.zeros((m_batchsize, c1, self.bl - L % self.bl)).to(q.device)], dim=-1)
            k = torch.cat(
                [k, torch.zeros((m_batchsize, c2, self.bl - L % self.bl)).to(k.device)], dim=-1)
            v = torch.cat(
                [v, torch.zeros((m_batchsize, c3, self.bl - L % self.bl)).to(v.device)], dim=-1)
            nb += 1

        padding_mask = torch.cat([
            torch.ones((m_batchsize, 1, L)).to(mask.device) * mask[:, 0:1, :],
            torch.zeros((m_batchsize, 1, self.bl * nb - L)).to(mask.device)
        ], dim=-1)
    ...
    def _sliding_window_self_att(self, q, k, v, mask):
        ...
        k = torch.cat([
            torch.zeros(m_batchsize, c2, self.bl // 2).to(k.device),
            k,
            torch.zeros(m_batchsize, c2, self.bl // 2).to(k.device)
        ], dim=-1)
        v = torch.cat([
            torch.zeros(m_batchsize, c3, self.bl // 2).to(v.device),
            v,
            torch.zeros(m_batchsize, c3, self.bl // 2).to(v.device)
        ], dim=-1)
        padding_mask = torch.cat([
            torch.zeros(m_batchsize, 1, self.bl // 2).to(padding_mask.device),
            padding_mask,
            torch.zeros(m_batchsize, 1, self.bl // 2).to(padding_mask.device)
        ], dim=-1)
        ...
        final_mask = self.window_mask.repeat(
            m_batchsize * nb, 1, 1).to(padding_mask.device) * padding_mask
...
class Decoder(nn.Module):
    ...
    def update_fcs(self, num_classes):
        ...
        num_f_maps, prev_num_classes, _ = self.conv_1x1.weight.shape

        # Update the input of the decoder block
        logging.info(
            f'Updating the decoder input heads from {prev_num_classes} to {num_classes}')
        weight = copy.deepcopy(self.conv_1x1.weight.data)
        bias = copy.deepcopy(self.conv_1x1.bias.data)
        new_conv_1x1 = nn.Conv1d(num_classes, num_f_maps, 1)
        new_conv_1x1.weight.data[:, :prev_num_classes, :] = weight
        new_conv_1x1.bias.data = bias
        del self.conv_1x1
        self.conv_1x1 = new_conv_1x1

        # Update the output of the decoder block
        logging.info(
            f'Updating the decoder output heads form {prev_num_classes} to {num_classes}')
        weight = copy.deepcopy(self.conv_out.weight.data)
        bias = copy.deepcopy(self.conv_out.bias.data)
        new_fc = nn.Conv1d(num_f_maps, num_classes, 1)
        new_fc.weight.data[:prev_num_classes] = weight
        new_fc.bias.data[:prev_num_classes] = bias
        del self.conv_out
        self.conv_out = new_fc
...
class MyTransformer(nn.Module):
    def __init__(self, num_decoders, num_layers, r1, r2, num_f_maps, input_dim, num_classes, channel_masking_rate):
        ...
        self.decoders = nn.ModuleList([
            copy.deepcopy(Decoder(num_layers, r1, r2, num_f_maps, num_classes,
                          num_classes, att_type='sliding_att', alpha=exponential_descrease(s)))
    ...
    def update_fcs(self, num_classes):
        self.encoder.update_fcs(num_classes)
        for decoder in self.decoders:
            decoder.update_fcs(num_classes)


class ASFormerTrainer(BaseTrainer):
    def __init__(self, num_decoders, num_layers, r1, r2, num_f_maps, input_dim, num_classes, channel_masking_rate):
        self.model = MyTransformer(num_decoders, num_layers, r1, r2,
                                   num_f_maps, input_dim, num_classes, channel_masking_rate)
    ...
    def calc_loss(self, predictions, batch_target, mask):
        ...
        for p in predictions:
            ...
            loss += 0.25*torch.mean(torch.clamp(self.mse(F.log_softmax(p[:, :, 1:], dim=1), F.log_softmax(
