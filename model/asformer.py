...
class AttLayer(nn.Module):
    ...
    def _block_wise_self_att(self, q, k, v, mask):
        ...
        if L % self.bl != 0:
            ...
            k = torch.cat(
                [k, torch.zeros((m_batchsize, c2, self.bl - L % self.bl)).to(k.device)],
                dim=-1,
            )
            v = torch.cat(
                [v, torch.zeros((m_batchsize, c3, self.bl - L % self.bl)).to(v.device)],
                dim=-1,
            )
            nb += 1
    ...
    def _sliding_window_self_att(self, q, k, v, mask):
        ...
        final_mask = (
            self.window_mask.repeat(m_batchsize * nb, 1, 1).to(padding_mask.device)
            * padding_mask
        )
