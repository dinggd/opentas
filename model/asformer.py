...
class AttLayer(nn.Module):
    ...
    def _normal_self_att(self,q,k,v, mask):
        ...
        _,c2,L = k.size()
        _,c3,L = v.size()
        padding_mask = torch.ones((m_batchsize, 1, L)).to(mask.device) * mask[:,0:1,:]
    ...
    def _sliding_window_self_att(self, q,k,v, mask):
        ...
        final_mask = self.window_mask.repeat(m_batchsize * nb, 1, 1).to(padding_mask.device) * padding_mask 
...
class Encoder(nn.Module):
    ...
    def update_fcs(self, num_classes):
        ...
        prev_num_classes, num_f_maps, _ = self.conv_out.weight.shape
        logging.info(
            f'Updating the encoder output heads form {prev_num_classes} to {num_classes}')
        
        # out shape change: update the last later that 
        # projects num_f_maps to num_classes
...
class MyTransformer(nn.Module):
    def __init__(self, num_decoders, num_layers, r1, r2, num_f_maps, input_dim, num_classes, channel_masking_rate):
        ...
        self.decoders = nn.ModuleList([
            copy.deepcopy(Decoder(num_layers, r1, r2, num_f_maps, num_classes, num_classes, att_type='sliding_att', alpha=exponential_descrease(s))) 
