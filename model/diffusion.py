...
class ASDiffusionModel(nn.Module):
    def __init__(self, encoder_params, decoder_params, diffusion_params, num_classes):
        ...
        if self.use_instance_norm:
            self.ins_norm = nn.InstanceNorm1d(
                encoder_params["input_dim"], track_running_stats=False
            )
        ...
        decoder_params["input_dim"] = (
            len(
                [
                    i
                    for i in encoder_params["feature_layer_indices"]
                    if i not in [-1, -2]
                ]
            )
    ...
    def model_predictions(self, backbone_feats, x, t):
        ...
        # torch.Size([1, C, T])
        x_start = self.decoder(backbone_feats, t, x_m.float())
    ...
    def prepare_targets(self, event_gt):
        ...
        noise = torch.randn(size=event_gt.shape).cuda()

        x_start = (event_gt * 2.0 - 1.0) * self.scale  # [-scale, +scale]

        # noise sample
        x = self.q_sample(x_start=x_start, t=t, noise=noise)

        x = torch.clamp(x, min=-1 * self.scale, max=self.scale)
        event_diffused = ((x / self.scale) + 1) / 2.0  # normalized [0, 1]

        return event_diffused, noise, t

    def forward(
        self, backbone_feats, t, event_diffused, event_gt=None, boundary_gt=None
    ):  # only for train
        ...
        if cond_type == "full":
            ...
            event_out = self.decoder(
                torch.zeros_like(backbone_feats), t, event_diffused.float()
            )

        elif cond_type == "boundary05-":
            feature_mask = (boundary_gt < 0.5).float()  # maybe try 0.1
            event_out = self.decoder(
                feature_mask * backbone_feats, t, event_diffused.float()
            )
            ...
            feature_mask = (event_gt != random_event_1).float() * (
                event_gt != random_event_2
            ).float()
            event_out = self.decoder(
                feature_mask * backbone_feats, t, event_diffused.float()
            )
    ...
    def get_training_loss(
        ...
        event_gt,
        boundary_gt,
        encoder_ce_criterion,
        encoder_mse_criterion,
        encoder_boundary_criterion,
        decoder_ce_criterion,
        decoder_mse_criterion,
        decoder_boundary_criterion,
        soft_label,
    ):
        ...
        encoder_boundary_loss = torch.tensor(0).cuda()  # No boundary loss for encoder
        ...
        event_out = self.forward(
            backbone_feats, t, event_diffused, event_gt, boundary_gt
        )

        decoder_boundary = 1 - torch.einsum(
            "bicl,bcjl->bijl",
        ...
        if soft_label is None:  # To improve efficiency
            ...
            soft_event_gt = torch.clone(event_gt).float().cpu().numpy()
            for i in range(soft_event_gt.shape[1]):
                soft_event_gt[0, i] = gaussian_filter1d(
                    soft_event_gt[0, i], soft_label
                )  # the soft label is not normalized
            soft_event_gt = torch.from_numpy(soft_event_gt).cuda()

            decoder_ce_loss = -soft_event_gt * F.log_softmax(event_out, 1)
            decoder_ce_loss = decoder_ce_loss.sum(0).sum(0)

        decoder_mse_loss = torch.clamp(
            decoder_mse_criterion(
        ...
        decoder_boundary_loss = decoder_boundary_criterion(
            decoder_boundary, boundary_gt[:, :, 1:]
        )
    ...
    def ddim_sample(self, video_feats, seed=None):
        ...
        for time, time_next in time_pairs:
            ...
            x_time = x_start * alpha_next.sqrt() + c * pred_noise + sigma * noise
...
class EncoderModel(nn.Module):
    def __init__(
        ...
        self.encoder = MixedConvAttModule(
            num_layers, num_f_maps, kernel_size, normal_dropout_rate
        )
    ...
    def forward(self, x, get_features=False):
        if get_features:
            assert (
                self.feature_layer_indices is not None
                and len(self.feature_layer_indices) > 0
            )
            features = []
            ...
            x = (
                self.dropout_temporal(x.unsqueeze(3).transpose(1, 2))
                .squeeze(3)
                .transpose(1, 2)
            )
            x, feature = self.encoder(
                self.conv_in(x), feature_layer_indices=self.feature_layer_indices
            )
            ...
            out = self.conv_out(
                self.encoder(self.conv_in(x), feature_layer_indices=None)
            )
            return out
...
class DecoderModel(nn.Module):
    def __init__(
        ...
        self.module = MixedConvAttModuleV2(
            num_layers, num_f_maps, input_dim, kernel_size, dropout_rate, time_emb_dim
        )
        self.conv_out = nn.Conv1d(num_f_maps, num_classes, 1)
...
class MixedConvAttentionLayerV2(nn.Module):
    ...
    def get_attn_indices(self, l):
        ...
        self.attn_indices = self.attn_indices.cuda()

    def attention(self, x, x_cross):

        if self.attn_indices is None:
            self.get_attn_indices(x.shape[2])
        else:
        ...
        x_k = torch.index_select(
            F.pad(x_k, (self.padding, self.padding), "constant", 0), 2, flat_indicies
        )
        x_v = torch.index_select(
            F.pad(x_v, (self.padding, self.padding), "constant", 0), 2, flat_indicies
        )
...
class MixedConvAttModule(nn.Module):  # for encoder
    def __init__(
        self, num_layers, num_f_maps, kernel_size, dropout_rate, time_emb_dim=None
    ):
