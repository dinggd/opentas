...
def get_data_dict(feature_dir, label_dir, video_list, actions_dict, sample_rate=4, temporal_aug=True, boundary_smooth=None):
    ...
    for video in tqdm(video_list):
        ...
        for i in range(frame_num):
            event_seq_raw[i] = actions_dict[event[i]]
...
def get_data_loader(cfg, mode=["train", "test"]):

    samples_file = cfg.DATA.VID_LIST_FILE_TEST if mode == "test" \
                   else cfg.DATA.VID_LIST_FILE

    with open(samples_file, "r") as file_ptr:
        video_list = [ vid.split(".")[0] for vid in file_ptr.read().split("\n")[:-1] ]

    data_dict = get_data_dict(
        feature_dir=cfg.DATA.FEATURES_PATH, 
        label_dir=cfg.DATA.GT_PATH, 
        ...
        boundary_smooth=cfg.TRAIN.BOUNDARY_SMOOTH
    )
